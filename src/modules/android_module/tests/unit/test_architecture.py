...
from modules.android_module.domain import ports as ports_module
...
MODULE_PACKAGE = "modules.android_module"
...
def _sample_arguments(error_type: type[BaseException]) -> tuple[object, ...]:
    ...
    from modules.android_module.infrastructure.android_emulator_manager import CommandResult
...
def test_the_server_defines_no_tool_of_its_own() -> None:
    ...
    source = INDEX_FILE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(INDEX_FILE))
    offenders = [

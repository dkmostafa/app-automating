...
from modules.appium_module.application import build_appium_device_manager
from modules.appium_module.domain import ports as ports_module
...
MODULE_PACKAGE = "modules.appium_module"
...
#: whatever the import spelling. `appium` and `selenium` matter most here: this
#: module is *about* Appium, which makes it the module most likely to let the
#: client library leak inward.
...
DOMAIN_FORBIDDEN_STDLIB = frozenset(
    {"subprocess", "socket", "shutil", "os", "asyncio", "urllib", "http"}
)

#: The four ports, and what the adapter must satisfy.
...
def test_the_domain_imports_on_a_host_with_no_appium() -> None:
    """The practical form of the rule above: import it and see.

    A domain that cannot be imported without the client library is a domain that
    has quietly become an adapter.
    """
    import importlib
...
def test_the_composition_root_returns_something_that_satisfies_the_ports() -> None:
    ...
    from modules.appium_module.application.di import appium_device_ports

    ports = appium_device_ports(build_appium_device_manager(AppiumConfig()))
...
def _error_classes() -> list[type[BaseException]]:
    ...
    for module_name in (
        ...
        f"{MODULE_PACKAGE}.infrastructure.appium_device_manager.errors",
    ):
...
def test_every_service_takes_its_collaborators_through_the_constructor() -> None:
    ...
    for name, parameter in inspect.signature(AppiumDeviceService.__init__).parameters.items():
        ...
        assert parameter.default is inspect.Parameter.empty, (
            f"AppiumDeviceService.{name} has a default; the composition root passes it in"
        )
...
SRC_ROOT = MODULE_ROOT.parents[1]
INDEX_FILE = SRC_ROOT / "index.py"
...
def test_the_presentation_layer_exports_a_register_function() -> None:
    ...
    assert _register_functions(), (
        "presentation/__init__.py exports no register_*_tools; index.py has nothing to call"
    )
...
def test_the_presentation_layer_builds_nothing() -> None:
    """Rule 0 §4 and Rule 3 §2: composition is di.py's job, called by index.py.
    ...
    """
    forbidden = ("build_appium", "from_environment", "AppiumConfig")
    offenders: list[str] = []
...
def test_the_server_registers_every_surface_this_module_exposes() -> None:
    ...
    ``src/index.py`` is read as text on purpose -- importing it would build the
    real object graph, and this file must keep running on a bare host.
    """
    assert INDEX_FILE.is_file(), f"expected the MCP server at {INDEX_FILE}"
    source = INDEX_FILE.read_text(encoding="utf-8")
    unregistered = [name for name in _register_functions() if f"{name}(" not in source]
    assert not unregistered, (
        f"{INDEX_FILE.name} never calls: {unregistered}. A module's tools reach the "
        "server through one import and one call."
    )


def test_the_server_defines_no_tool_of_its_own() -> None:
    """Rule 3 §1: a `@mcp.tool` in index.py is a presentation layer never written."""
    tree = ast.parse(INDEX_FILE.read_text(encoding="utf-8"), filename=str(INDEX_FILE))
    offenders = [
        f"{INDEX_FILE.name}:{node.lineno} defines a tool"
        for node in ast.walk(tree)
...
def test_the_server_never_reaches_into_a_modules_infrastructure() -> None:
    """Rule 3 §1: index.py wires through di.py, and names no adapter."""
    tree = ast.parse(INDEX_FILE.read_text(encoding="utf-8"), filename=str(INDEX_FILE))
    offenders = [
        f"{INDEX_FILE.name}:{node.lineno} imports {node.module}"
        for node in ast.walk(tree)
        if isinstance(node, ast.ImportFrom) and "infrastructure" in (node.module or "")
    ]
    offenders += [
        f"{INDEX_FILE.name}:{node.lineno} imports {alias.name}"
        for node in ast.walk(tree)
        if isinstance(node, ast.Import)
        for alias in node.names

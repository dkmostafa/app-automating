...
from modules.navigation_memory_module.application.di import (
...
def test_the_port_decorator_is_built_here_and_not_by_index() -> None:
    """Rule 3 §1: ``src/index.py`` may not name an adapter, so the one function
...
async def test_record_interactions_off_yields_no_decorator() -> None:
    """``src/index.py`` wires ``appium_module`` with no recording at all rather

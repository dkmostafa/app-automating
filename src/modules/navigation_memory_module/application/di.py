...
one port that leaves this module: ``src/index.py`` uses it to build the
...
def build_appium_port_decorator(recorder: RouteRecorder) -> AppiumPortDecorator:
    ...
    the composition root's job and nobody else's -- ``src/index.py`` may not
    import an ``infrastructure`` package (Rule 3 §1), so the one function that
    knows the two modules can be connected lives here. ``src/index.py`` passes
